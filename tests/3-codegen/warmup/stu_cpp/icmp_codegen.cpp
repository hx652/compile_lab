...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("slt $t1, $t0, $zero");
    codegen->append_inst("slt $t2, $zero, $t0");
    codegen->append_inst("xor $t2, $t1, $t2");
    codegen->append_inst("st.w", {"$t2", "$fp", std::to_string(offset_map["%op2"])});
