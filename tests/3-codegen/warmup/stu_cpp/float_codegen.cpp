...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("store float 0x40091eb860000000, float* %op0", ASMInstruction::Comment);
    ...
    codegen->append_inst("%op1 = load float, float* %op0", ASMInstruction::Comment);
    ...
    codegen->append_inst("%op2 = fptosi float %op1 to i32", ASMInstruction::Comment);
    ...
    codegen->append_inst("fst.s", {"$ft0", "$fp", std::to_string(offset_map["%op2"])});
