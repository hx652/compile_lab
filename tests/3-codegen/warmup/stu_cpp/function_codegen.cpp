...
void translate_callee(CodeGen *codegen) {
    ...
    codegen->append_inst("st.w", {"$a0", "$fp", std::to_string(offset_map["%arg0"])});
    ...
    codegen->append_inst("ld.w", {"$t1", "$fp", std::to_string(offset_map["%arg0"])});
    ...
    // 将 %op3 的值写入 $a0 中
    codegen->append_inst("ld.w", {"$a0", "$fp", std::to_string(offset_map["%op3"])});
...
void translate_main(CodeGen *codegen) {
    ...
    // 将返回值写入 %op0 对应的内存空间中
    codegen->append_inst("st.w", {"$a0", "$fp", std::to_string(offset_map["%op0"])});
